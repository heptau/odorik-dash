			...
			try {
				...
				if (registration) {
					...
					setTimeout(() => {
						...
						setUpdateInfo('PWA is active and up to date');
					}, 1000);
				} else {
					setUpdateStatus('no-sw');
					setUpdateInfo('PWA not installed');
				}
			} catch {
				setUpdateStatus('no-sw');
			}
					...
					{showAboutDetails && (
						<div className="px-4 pb-4 space-y-3">
							...
							<div className="text-sm space-y-2" style={{ color: 'var(--text-secondary)' }}>
								...
								<a href="https://github.com/heptau/odorik-dash" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm" style={{ color: 'var(--accent)' }}>
							...
							{/* Update Check - PWA Service Worker */}
							<div className="pt-2 mt-2" style={{ borderTop: '0.5px solid var(--separator)' }}>
								<div className="flex items-center justify-between gap-2">
									<span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
										{updateStatus === 'idle' && t('settings.update_check')}
										{updateStatus === 'checking' && t('settings.update_checking')}
										{updateStatus === 'available' && (updateInfo || t('settings.update_available'))}
										{updateStatus === 'up-to-date' && (updateInfo || t('settings.update_latest'))}
									...
									{updateStatus === 'up-to-date' && (
										<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" style={{ color: 'var(--success)' }}>
											<path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
										</svg>
									)}
								</div>
